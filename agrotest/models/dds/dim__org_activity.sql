{{ config(
    ...
    post_hook="
       alter table {{ this }}
       add primary key (org_activity_sk)
    "
) }}
...
with marked_doubles_data as (
	...
	select
			...
			start_dt,
			end_dt,
			lag(activity_code_main, 1 ,activity_code_main) over (
				partition by vdd.org_sk
				order by start_dt
			) as
				lag_activity_code_main,
	...
	select
			...
			case
				when
					activity_code_main = lag_activity_code_main and
					start_dt = lag_end_dt
				then
					0
				else
...
select
		uuid_generate_v5(
			...
			org_activity_sk,
		org_sk,
		min(activity_code_main) as activity_code_main,
